...
elicbdf["V_LN"] = F["elicb_mvln"]
elicbdf["V_LE"] = F["elicb_mvle"]
elicbdf["V_NE"] = F["elicb_mvne"]
elicbdf["L1-ELI"] = F["elicb_mel1"]
elicbdf["L2-ELI"] = F["elicb_mel2"]
elicbdf["L3-ELI"] = F["elicb_mel3"]
...
gf_filled = elicbdf.fillna("")
elicbdf["Device Rating (A)"] = elicbdf["Device Rating (A)"].astype(int)

Device_Rating = elicbdf["Device Rating (A)"]
No_phase = elicbdf["No. of Phases"]
T_Curve = elicbdf["Trip Curve"]
new_column1 = []
result_column1 = []
P = 0
K = 0
...
def eli_test_result1(gf1, gf2):
    I = Is * ((((K * TMS) / Td) + 1) ** (1 / P))
    if row["Earthing Configuration"] in ["TN", "TNS", "TNC", "TNCS"]:
...
for index, row in elicbdf.iterrows():
    if row["Device Type"] == "MCB":
        ...
        trip = row["Trip Curve"]  # Assuming the "Trip Curve" column is at index 10
        ...
        elif row["Trip Curve"] == "IEC Very Inverse":
            P = 1
            K = 13.5
            eli_test_result1(gf1, gf2)
        elif row["Trip Curve"] == "IEC Long-Time Inverse":
            P = 1
            K = 120
            eli_test_result1(gf1, gf2)
        elif row["Trip Curve"] == "IEC Extremely Inverse":
            P = 2
            K = 80
            eli_test_result1(gf1, gf2)
        elif row["Trip Curve"] == "IEC Ultra Inverse":
            P = 2.5
            K = 315.2
            eli_test_result1(gf1, gf2)
        elif row["Trip Curve"] == "IEEE Moderately Inverse":
            A = 0.0515
            B = 0.114
            p = 0.02
            eli_test_result2(gf1, gf2)
        elif row["Trip Curve"] == "IEEE Very Inverse":
            A = 19.61
            B = 0.491
            p = 2
            eli_test_result2(gf1, gf2)
        elif row["Trip Curve"] == "IEEE Extremely Inverse":
            A = 28.2
            B = 0.1217
            p = 2
            eli_test_result2(gf1, gf2)
...
def eli_test_combined_graph(elicbdf):
...
def main():
    ...
    graph_combined = eli_test_combined_graph(elicbdf)
    doc.add_picture(graph_combined, width=Inches(8), height=Inches(4))
    doc.save("ELIcb_Report.docx")
